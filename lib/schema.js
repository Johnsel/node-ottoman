...
function _decodeValue(context, type, data) {
  if (data instanceof Object && data.$ref) {
    ...
    if ((modelType.type && modelType.type === 'Mixed') ||
      ...
      modelType = context.typeByName(data.$type);

      if (!modelType) {
        throw new Error('Invalid type in mixed reference (' + data.$type + ')');
      }
    }

    return modelType.ref(data.$ref);
  } else if (data instanceof Object && data.$type) {
    if (type === mixedCoreType) {
      type = context.typeByName(data.$type);
      if (!type) {
...
Schema.coreTypeByName = function (type) {
  var coreType = $typeByNameLkp[type];
  if (coreType) {
    return coreType;
  }
  return null;
